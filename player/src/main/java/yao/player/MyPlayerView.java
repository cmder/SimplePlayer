...
import yao.player.MyControlView.SeekDispatcher;

import static android.content.Context.AUDIO_SERVICE;

/**
 * A high level view for {@link SimpleExoPlayer} media playbacks. It displays video, subtitles and
 * album art during playback, and displays playback controls using a {@link MyControlView}.
 * <p>
 ...
 * inflated by the child {@link MyControlView}. See below for more details.
 * <ul>
 * <li>Corresponding method: None</li>
 * <li>Default: {@code R.id.exo_playback_control_view}</li>
 * </ul>
 * <li>All attributes that can be set on a {@link MyControlView} can also be set on a
 * SimpleExoPlayerView, and will be propagated to the inflated {@link MyControlView}.
 * </li>
 * </ul>
 * <p>
 ...
 */
@TargetApi(16)
public final class MyPlayerView extends FrameLayout implements GestureView.PlayerOnGestureListener {
    ...
    private final MyControlView controller;
    private final ComponentListener componentListener;
    private final FrameLayout overlayFrameLayout;
    private final TextView exoInfo;
    private final GestureView underlay;
    ...
    public MyPlayerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        int controllerShowTimeoutMs = MyControlView.DEFAULT_SHOW_TIMEOUT_MS;
        if (attrs != null) {
        ...
        if (controllerPlaceholder != null) {
            ...
            // calls to set them.
            this.controller = new MyControlView(context, attrs);
        ...
        // control information
        exoInfo = (TextView) findViewById(R.id.exo_info);

        // gesture
        underlay = (GestureView) findViewById(R.id.exo_controller_underlay);
        underlay.setPlayerOnGestureListener(this);
    }
    ...
    /**
     * Set the {@link MyControlView.VisibilityListener}.
     *
     * @param listener The listener to be notified about visibility changes.
     */
    public void setControllerVisibilityListener(MyControlView.VisibilityListener listener) {
        Assertions.checkState(controller != null);
        controller.setVisibilityListener(listener);
    }

    /**
     * Set the {@link MyControlView.InterceptFinishCallback}.
     *
     * @param callback
     */
    public void setControllerInterceptFinishCallback(MyControlView.InterceptFinishCallback callback) {
        Assertions.checkState(controller != null);
        controller.setInterceptFinishCallback(callback);
    }
     ...
     *                       {@link MyControlView#DEFAULT_SEEK_DISPATCHER}.
     */
    ...
    private float mVolume; // 0 - mVolumeMax

    private float mScreenBrightness = 0.6f; // 0 - 1
    ...
    private void initScreenBrightness() {
        ...
        try {
            if (Settings.System.getInt(activity.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS_MODE)
            ...
            if (layoutParams.screenBrightness == -1.0f) {
                mScreenBrightness = Settings.System.getInt(activity.getContentResolver()
    ...
    private void setVolumeUp(float percentage) {
        float delta = percentage * mVolumeMax;
        mVolume += delta;
        ...
        if (delta != 0f) {
            ...
            showInfo(getContext().getString(R.string.volume_changing, (int) (mVolume * 100 / mVolumeMax))
                    , R.drawable.ic_volume);
        }
    }

    private void setVolumeDown(float percentage) {
        float delta = percentage * mVolumeMax;
        mVolume -= delta;
        if (mVolume < 0) {
            mVolume = 0;
    ...
    private void setScreenBrightnessUp(float percentage) {
        if (!(getContext() instanceof Activity)) {
            return;
        }
        Activity activity = (Activity) getContext();
        ...
        if (percentage != 0) {
            ...
            showInfo(getContext().getString(R.string.brightness_changing, (int) Math.round(mScreenBrightness * 100))
                    , R.drawable.ic_brightness);
        }
    }

    private void setScreenBrightnessDown(float percentage) {
        if (!(getContext() instanceof Activity)) {
            return;
        }
        Activity activity = (Activity) getContext();
    ...
    private void setProgressForward(float percentage) {
        if (percentage == 0 || !controller.isSeekable()) {
            return;
        }
        ...
        long seekToPosition = (currentPosition + duration * percentage) > duration
                ? duration : (currentPosition + (long) (duration * percentage));
        seek(seekToPosition);
        exoInfo.setVisibility(VISIBLE);
        exoInfo.setText(generateProgressInfo(seekToPosition));
        exoInfo.setCompoundDrawablesWithIntrinsicBounds(null, getContext().getResources().getDrawable(R.drawable.ic_fast_forward), null, null);
    }

    private void setProgressBackward(float percentage) {
        if (percentage == 0 || !controller.isSeekable()) {
            return;
        }
        long duration = player.getDuration();
        ...
        long currentPosition = player.getCurrentPosition();
        long seekToPosition = (currentPosition - duration * percentage) < 0
                ? 0 : (currentPosition - (long) (duration * percentage));
        seek(seekToPosition);
        exoInfo.setVisibility(VISIBLE);
        exoInfo.setText(generateProgressInfo(seekToPosition));
        exoInfo.setCompoundDrawablesWithIntrinsicBounds(null, getContext().getResources().getDrawable(R.drawable.ic_fast_rewind), null, null);
    }

    private CharSequence generateProgressInfo(long position) {
        long duration = player == null ? 0 : player.getDuration();
        String result = controller.stringForTime(position) + " / " + controller.stringForTime(duration);
        int index = result.indexOf("/");
        SpannableString spannableString = new SpannableString(result);
        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#FF4081")), 0, index, Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
    ...
    private void showInfo(String txt, @DrawableRes int drawableId) {
        exoInfo.setVisibility(VISIBLE);
        exoInfo.setText(txt);
        exoInfo.setTextColor(getContext().getResources().getColor(android.R.color.white));
        exoInfo.setCompoundDrawablesWithIntrinsicBounds(null, getContext().getResources().getDrawable(drawableId), null, null);
    }

    private void hideInfo() {
        if (exoInfo.getVisibility() == VISIBLE) {
            exoInfo.startAnimation(AnimationUtils.loadAnimation(getContext(), android.R.anim.fade_out));
            exoInfo.setVisibility(GONE);
        }
    }
    ...
    public void onDoubleTap() {
        if (player.getPlaybackState() == ExoPlayer.STATE_READY) {
            player.setPlayWhenReady(player.getPlayWhenReady() ? false : true);
        }
    }

    @Override
    public void onSlideUpLeft(float percentage) {
        setScreenBrightnessUp(percentage);
    }

    @Override
    public void onSlideDownLeft(float percentage) {
        setScreenBrightnessDown(percentage);
    }

    @Override
    public void onSlideUpRight(float percentage) {
        setVolumeUp(percentage);
    }

    @Override
    public void onSlideDownRight(float percentage) {
        setVolumeDown(percentage);
    }

    @Override
    public void onSlideLeft(float percentage) {
        setProgressBackward(percentage);
    }

    @Override
    public void onSlideRight(float percentage) {
        setProgressForward(percentage);
    }
    ...
    private final class ComponentListener implements SimpleExoPlayer.VideoListener,
        ...
        public void onRenderedFirstFrame() {
            ...
            setTitle(names[player.getCurrentWindowIndex()]);
        }
