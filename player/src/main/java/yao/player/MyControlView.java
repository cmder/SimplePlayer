 ...
 */
public class MyControlView extends FrameLayout {
    ...
    private final ImageView exoBack;
    private final TextView exoTitle;
    private final View playBigButton;
    private final View pauseBigButton;
    ...
    public MyControlView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            try {
                ...
                title = a.getString(R.styleable.PlaybackControlView_title);
        ...
        exoBack = (ImageView) findViewById(R.id.exo_back);
        if (exoBack != null) {
            exoBack.setOnClickListener(componentListener);
        }
        ...
        playBigButton = findViewById(R.id.exo_play_big);
        if (playBigButton != null) {
            playBigButton.setOnClickListener(componentListener);
        }

        pauseBigButton = findViewById(R.id.exo_pause_big);
        if (pauseBigButton != null) {
            pauseBigButton.setOnClickListener(componentListener);
        }
    }
    ...
    private void updatePlayPauseButton() {
        ...
        if (playButton != null) {
            ...
            if (playBigButton != null) {
                playBigButton.setVisibility(playing ? View.GONE : View.VISIBLE);
            }
        }
        if (pauseButton != null) {
            ...
            if (pauseBigButton != null) {
                pauseBigButton.setVisibility(!playing ? View.GONE : View.VISIBLE);
            }
        }
    ...
    /**
     * 这里将权限从private改为public，为了让MyPlayerView调用
     *
     * @param timeMs
     * @return
     */
        ...
        public void onClick(View view) {
            if (player != null) {
                if (nextButton == view) {
                    ...
                    player.setPlayWhenReady(false);
                } else if (playBigButton == view) {
                    player.setPlayWhenReady(true);
                } else if (pauseBigButton == view) {
                    player.setPlayWhenReady(false);
                } else if (exoBack == view) {
                    if (mInterceptFinishCallback != null) {
                        mInterceptFinishCallback.interceptFinish();
                    } else {
    ...
    private InterceptFinishCallback mInterceptFinishCallback;

    public InterceptFinishCallback getInterceptFinishCallback() {
        return mInterceptFinishCallback;
    }

    public void setInterceptFinishCallback(InterceptFinishCallback interceptFinishCallback) {
        this.mInterceptFinishCallback = interceptFinishCallback;
    }

    /**
     * 用于拦截返回事件的接口
     */
    public interface InterceptFinishCallback {
